...
class Server:
    ...
    def parse_header(self, header_content):
        ...
        filename = header[0].split(": ")[1].replace(",", "")
        filesize = int(header[1].split(": ")[1])
    ...
    def start(self):
        ...
        try:
            while True:
                ...
                for self.server_socket in read_ready:
                    # if socket ready is the server socket
                    if read_ready == self.server_socket:
                        # accept connection
                        ...
                        try:
                            ...
                            self.input_socket.remove(self.server_socket)
                            break
                        ...
                        if command != "upload":
                            ...
                            self.server_socket.sendall(b"Unknown command")
                            continue
                            ...
                            file_name, file_size, content = self.parse_header(
                                data.decode())
                            file_path = os.path.join(BASE_DIR, file_name)

                            # receive_file
                            self.receive_file(
                                content, file_path, file_size, self.server_socket)
